...
pub(crate) struct Keyboard {
    ...
    /// Connection to the X-server to read-data
    data_conn:           Arc<RustConnection>,
    /// ID for `record_get_context`
    id:                  u32,
...
impl Keyboard {
    ...
    pub(crate) fn gen_record_ctx(&self) -> Result<()> {
        ...
        self.xcape
            .ctrl_conn()
            .record_create_context(self.xcape.id(), 0, &[record::CS::ALL_CLIENTS.into()], &[
                range,
            ])
            ...
            cthread::scope(|scope| {
                scope.spawn(|_| {
                    ...
                    self.xcape
                        .ctrl_conn()
                        .sync()
                        .expect("failed to sync X-Server");
                });
            });
        };
    ...
    pub(crate) fn run(&self, state: &mut XcapeState) -> Result<()> {
        ...
        for reply in self
            .xcape
            .data_conn()
            .record_enable_context(self.xcape.id())
