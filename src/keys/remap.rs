...
pub(crate) struct RemapKeyState {
    ...
    // If the key is held with another key, it is considered to be a modifier
    is_held:   AtomicBool,
...
impl RemapKeyState {
    ...
    pub(crate) fn from_chains(from: &Chain, to: &Chain) -> Option<Self> {
        ...
        Some(Self {
            from_key:     from.chords()[0].charmap().clone(),
            to_keys:      to
                .chords()
                .iter()
            ...
            mouse_button: AtomicU8::new(from.chords()[0].button().code()),
            generated:    AtomicBool::new(false),
            is_held:      AtomicBool::new(
                to.chords()
                    .iter()
    ...
    pub(crate) fn from_key(&self) -> &CharacterMap {
        &self.from_key
    }
...
impl RemapState {
    ...
    pub(crate) fn mark_released(&mut self, key: Keycode) -> Option<(bool, bool)> {
        self.remapped_keys
            ...
            .find(|m| m.from_key().code() == key)
            .map(|ref mut map| {
                let old = (&map.pressed, &map.is_held);
                log::debug!(
                    "{}: {} => {}; {}: ({},{}) => (false,false)",
                    "updated key".green().bold(),
                    map.from_key().utf(),
                ...
                map.is_held.store(false, Ordering::Relaxed);
    ...
    pub(crate) fn set_modifier(&mut self) {
        self.remapped_keys
            ...
            .for_each(|mut to| {
                to.is_held.store(true, Ordering::Relaxed);
            });
    }
    ...
    pub(crate) fn check_if_generated(&mut self, key: Keycode) -> bool {
        if let Some(ref mut map) = &mut self.remapped_keys.iter().find(|m| {
            m.to_keys().iter().any(|to| to.code() == key) && m.generated.load(Ordering::Relaxed)
        }) {
            log::debug!(
